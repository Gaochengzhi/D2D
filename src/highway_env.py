...
config_path = (
    ...
    + "/../../../Environment/environment/env5/Navigation.sumocfg"
)  
...
class Highway_env(gym.Env):
    ...
    def raw_obs(self, vehicle_params):  # dimension: 24+5
        ...
        if self.AutoCarID in vehicle_params:
            ...
            dis_goal_ego = np.linalg.norm(
                np.array([self.x_goal - ego_veh_x, self.y_goal - ego_veh_y])
            )
            ...
            for VehID in vehicle_params:
                ...
                if VehID != self.AutoCarID and dis < self.maxDistance:
                    angle = math.degrees(
                        math.atan2(veh_y - ego_veh_y, veh_x - ego_veh_x)
                    )
                    ...
                    if 0 <= angle < math.degrees(math.atan2(3**0.5, 1)):  # 0~60
                        ...
                        math.degrees(math.atan2(3**0.5, 1))
                        <= angle
                        < math.degrees(math.atan2(3**0.5, -1))
                    ):  # 60~120
                        ...
                        -180 <= angle < math.degrees(math.atan2(-(3**0.5), -1))
                    ):  # -180~-120
    ...
    def obs_to_state(self, vehicle_params):
        ...
        # print("raw_obs===>", obs)
        state = [
            obs[0] / self.maxDistance,
            obs[1] / self.max_angle,
            obs[2] / self.maxSpeed,
            obs[3] / self.max_angle,
            obs[4] / self.maxDistance,
            obs[5] / self.max_angle,
            obs[6] / self.maxSpeed,
            obs[7] / self.max_angle,
            obs[8] / self.maxDistance,
            obs[9] / self.max_angle,
            obs[10] / self.maxSpeed,
            obs[11] / self.max_angle,
            obs[12] / self.maxDistance,
            obs[13] / self.max_angle,
            obs[14] / self.maxSpeed,
            obs[15] / self.max_angle,
            obs[16] / self.maxDistance,
            obs[17] / self.max_angle,
            obs[18] / self.maxSpeed,
            obs[19] / self.max_angle,
            obs[20] / self.maxDistance,
            obs[21] / self.max_angle,
            obs[22] / self.maxSpeed,
            obs[23] / self.max_angle,
            obs[24] / self.maxSpeed,
            obs[25] / self.max_angle,
            obs[26] / self.maxDistance,
            obs[27],
    ...
    def step(self, action_a):
        ...
        traci.vehicle.setSpeed(
            self.AutoCarID,
            max(traci.vehicle.getSpeed(self.AutoCarID) + control_acc, 0.001),
        )
        ...
        ) = self.get_reward(new_vehicle_params)
        ...
        for VehId in VehicleIds:
            if VehId == self.AutoCarID:
                ...
                traci.vehicle.setLaneChangeMode(
                    VehId, 1
                )  # Disable automatic lane changing
    ...
    def start(self, gui=False):
        ...
        traci.start(
            [sumoBinary, "-c", config_path, "--collision.check-junctions", "true"]
        )
